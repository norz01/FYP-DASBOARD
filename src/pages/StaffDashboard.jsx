...
export default function StaffDashboard() {
  ...
  const handleMdbUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMdbFile(null);
        setTimeout(() => window.location.reload(), 2000); // Reload dashboard to see new data
      } else {
        setUploadMsg({
          type: "error",
          text: result.message || "Ralat semasa memuat naik.",
        });
      }
    } catch (error) {
  ...
  const navItems = [
    ...
    { id: "data", icon: "ph-database", label: "Pengurusan Data" }, // NEW TAB
  ];
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative w-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-20">
          ...
          {activeTab === "management" && (
            <StudentListGrid
              students={students}
              onViewProfile={(id) => navigate(`/student-profile?id=${id}`)}
              onAddStudent={() => {
                setEditingStudent(null);
                setFormData({
                  ID_Pelajar: "",

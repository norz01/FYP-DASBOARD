...
export default function StudentDashboard() {
  ...
  const isRestrictedUser =
    currentUser?.role === "user" && currentUser?.studentId;
  ...
  // Certificate State (Kini ditarik dari API)
  ...
  useEffect(() => {
    ...
    const loadStudentData = async () => {
      try {
        // Dapatkan data chart & AI insight
        const resGap = await fetch(
          `/api/students/${selectedStudentId}/skill-gap`,
          {
            headers: { Authorization: `Bearer ${getToken()}` },
          },
        );
        ...
        // Dapatkan data penuh pelajar secara berasingan untuk pastikan sijil diambil
  ...
  // Fungsi Baharu: Hantar sijil ke Backend
    ...
    try {
      const res = await fetch(
        `/api/students/${selectedStudentId}/certificates`,
        {
          ...
          headers: {
            Authorization: `Bearer ${getToken()}`,
            // Note: Jangan set "Content-Type" secara manual untuk FormData, browser akan buat sendiri
          },
          body: formData,
        },
      );
  ...
  // Fungsi Baharu: Padam sijil dari Backend

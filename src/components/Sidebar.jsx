...
export default function Sidebar({ navItems, activeTab, setActiveTab, isSidebarOpen, setIsSidebarOpen, currentUser, handleLogout }) {
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-white border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="flex flex-col">
                            <p className="text-sm font-bold text-slate-800 truncate w-32">{displayName}</p>
                            <p className="text-[10px] text-slate-500 font-bold uppercase">{roleLabel}</p>
                        </div>
                    </div>
                    <button onClick={handleLogout} className="text-slate-400 hover:text-red-600 transition p-2 hover:bg-red-50 rounded-lg">
